...
public class ClassLoaderInjector {
    ...
    private static ClassLoader createNewClassLoader(Context context, ClassLoader oldClassLoader, List<File> patchs) throws Throwable {
         ...
         */
        //得到原本的pathList
        Field pathListField = HotfixReflectUtil.findField(oldClassLoader, "pathList");
        Object oldPathList = pathListField.get(oldClassLoader);

        //dexElements
        Field dexElementsField = HotfixReflectUtil.findField(oldPathList, "dexElements");
        Object[] oldDexElements = (Object[]) dexElementsField.get(oldPathList);

        //从Element上得到 dexFile
        Field dexFileField = HotfixReflectUtil.findField(oldDexElements[0], "dexFile");
        ...
        Field nativeLibraryDirectoriesField = HotfixReflectUtil.findField(oldPathList, "nativeLibraryDirectories");
    ...
    private static void doInject(Application app, ClassLoader classLoader) throws Throwable {
        ...
        Context baseContext = (Context) HotfixReflectUtil.findField(app, "mBase").get(app);
        if (Build.VERSION.SDK_INT >= 26) {
            HotfixReflectUtil.findField(baseContext, "mClassLoader").set(baseContext, classLoader);
        }

        Object basePackageInfo = HotfixReflectUtil.findField(baseContext, "mPackageInfo").get(baseContext);
        HotfixReflectUtil.findField(basePackageInfo, "mClassLoader").set(basePackageInfo, classLoader);

        if (Build.VERSION.SDK_INT < 27) {
            Resources res = app.getResources();
            try {
                HotfixReflectUtil.findField(res, "mClassLoader").set(res, classLoader);

                final Object drawableInflater = HotfixReflectUtil.findField(res, "mDrawableInflater").get(res);
                if (drawableInflater != null) {
                    HotfixReflectUtil.findField(drawableInflater, "mClassLoader").set(drawableInflater, classLoader);
                }
