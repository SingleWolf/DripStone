...
import com.walker.aop_optimize_plugin.utils.insertArgument
import com.walker.aop_optimize_plugin.utils.nameWithDesc
import com.walker.aop_optimize_plugin.utils.simpleClassName
import org.objectweb.asm.ClassReader
import org.objectweb.asm.ClassWriter
import org.objectweb.asm.Opcodes
import org.objectweb.asm.tree.*

class ShadowThreadTransform(private val config: ShadowThreadConfig) : BaseTransform() {
    companion object {
    ...
    private fun transformInvokeStatic(
        ...
        if (pointMethod != null) {
            ...
            methodInsnNode.owner = config.formatShadowThreadPoolClass
    ...
    private fun transformNew(
        ...
        for (index in typeInsnNodeIndex + 1 until instructions.size()) {
            ...
            if (node is MethodInsnNode && node.isThreadInitMethodInsn()) {
                //将 Thread 替换为 ShadowThread
                typeInsnNode.desc = config.formatShadowThreadClass
                node.owner = config.formatShadowThreadClass
                //为调用 Thread 构造函数的指令多插入一个 String 类型的方法入参参数声明
                node.insertArgument(String::class.java)
                //将 ClassName 作为构造参数传给 ShadowThread
