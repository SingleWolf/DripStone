...
import com.walker.aop_optimize_plugin.utils.findLambda
import com.walker.aop_optimize_plugin.utils.hasAnnotation
import com.walker.aop_optimize_plugin.utils.isStatic
import com.walker.aop_optimize_plugin.utils.nameWithDesc
import org.objectweb.asm.*
import org.objectweb.asm.tree.*

class AntiShakeTransform(private val config: AntiShakeConfig) : BaseTransform() {
    ...
    private companion object {
        ...
        private fun MethodNode.hasCheckViewAnnotation(config: AntiShakeConfig): Boolean {
            return hasAnnotation(config.formatCheckViewOnClickAnnotation)
        }

        private fun MethodNode.hasUncheckViewOnClickAnnotation(config: AntiShakeConfig): Boolean {
                            ...
                            if (instructions != null && instructions.size() > 0) {
                                ...
                                list.add(
                                    MethodInsnNode(
                                        ...
                                        config.formatShakeCheckClass,
                                        config.shakeCheckMethodName,
                                        config.shakeCheckMethodDescriptor
                                    )
                                )
    ...
    private fun ClassNode.isHookPoint(config: AntiShakeConfig, methodNode: MethodNode): Boolean {
    ...
    private fun MethodNode.findHookPointLambda(config: AntiShakeConfig): List<InvokeDynamicInsnNode> {
