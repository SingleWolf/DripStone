...
import com.walker.demo.feedback.FeedbackLogoFloat

class ScreenShotFloat(var context: Context) : BaseFloatView(context) {
    override fun getTag() = "ScreenShotFloatView"

    override fun show() {
        LogHelper.get()
            .i(FeedbackLogoFloat.TAG, "screenWith=${DisplayUtils.getScreenWidth(context)}")
        if (isShow()) {
